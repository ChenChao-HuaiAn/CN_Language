...
**最近交接**：2026-09-16——**246-a 双裁决落地 + D20 根治**：①B3 口径排除落盘（plans/026 M1 注记·分母 176·判据=可测格 100%）；②D17 wrapper 根治（AST 合成·344 转正·退出码语义写入 plans/001 附录B+spec 07）；③D20 根治（emitShift 掩码·重采 500/500 收敛）+CN-Smith 负向注入（30/30）。

### 一、本轮做了什么（写给无上下文的新会话）

1. **D20 根治**：x64 emitShift 常量路径按操作数位宽取模后发射（对齐第四十三轮 arm64 掩码修复）——重采 500 程序 **一致 500/500**（修复前 109 败全消·零分歧）+首采 4 样本收敛验证。
2. **D17 根治（用户裁决退出码语义）**：driver_module 合并后 **AST 合成整32 wrapper**——主_impl 调用+正常→.值（截断 int）/错误→.错误 码；344 转正（? 解包输出 7·rc=0）+错误路径探针 rc=3；**语义写入 plans/001 附录B 扩展+spec 07**。实现注意：mergeModules 后合成（流过完整语义/IR/优化器管线·平台无关）；兜底返回 0 满足全路径返回检查。
3. **B3 落盘**：plans/026 M1 行改「可测格口径」+口径注记（A 类 7 格排除·分母 176）+变更记录。
4. **CN-Smith 负向注入**：gen.py 加 `--negative` 模式（5 类注入：删分号/缺括号/缺花括号/非法字符/截断）——30 程序全部被诊断拒绝、零崩溃（T6 族机械化防线验证）。

### 二、验证链（全绿）

全量 ci.ps1：单测 1327/1327+E2E 375=375/0/0（344 转正后）；--ci PASS（84.7%）；CN-Smith 重采 500/500+负向 30/30。

### 三、下一步（新会话按序）

1. **A10 第五波（M1 收官）**：可测格口径 90.3%（159/176）——残余 17 格逐格补（C 类余量）+规范冻结清单（宏名登记制收紧呈报等）；--strict 口径切换。
2. **M3 CN-Smith 扩展**：宿主 vs v2 跨实现差分（待 v2p 工具链脚本化）、浮点/容器/字符串生成面。
3. **D18 升硬错误评估**（stdlib 已清零·D19 完成）；**D12**（v2 预处理·独立大轮）。
4. M1 收官后：F1-26+基准门禁（M4）→安全矩阵+ASan（M5）→M6/M8。

### 四、诚实边界

- D17 wrapper 的 主_impl 改名：用户显式调用 主() 的场景会失配（入口函数被显式调用罕见——诚实边界登记）。
- 500 重采与首轮共用 work 目录（产物覆盖——报告以 v2 为准）。
