...
> **交接原因**：2026-08-14 会话结束（**阶段A+B+C 串联集成验证完成**——确认 ARM64 后端/优化 Pass/寄存器分配/调试信息全部接入生产链路；修复 2 处链路断裂：① parser 表达式节点缺 location 导致 `--debug` 注释全被过滤（源头断裂，两平台受影响）、② debug_info 双前缀注释 `// // src:`；-O0/-O2 E2E 28/28 输出一致、964/964 单测、编译零警告）。本文档写给完全没有上下文的新会话看，帮助快速恢复开发。
>
> **前序里程碑**：阶段3「OOP 与错误处理」（901/901 单测、28/28 E2E）；阶段A「Linux ARM64」（918/918 单测、28/28 E2E）；阶段B 优化补全（949/949 单测、28/28 E2E）；阶段C 寄存器分配/调试信息（964/964 单测、28/28 E2E）；**阶段C 串联集成验证（本交接）**。
>
> **⚠️ 本次新增踩坑教训（务必注意）**：parser 表达式节点创建时**必须设置 location**（CallExpr/IdentifierExpr/字面量/复合表达式等），否则 IR 指令 `inst.loc` 无效 → `--debug` 注释、未来 DWARF 定位全部失效且无任何报错（静默断裂）。debug_info 的 `commentFor` 只应返回纯文本，注释语法前缀由各后端 AsmWriter 统一添加（避免双前缀）。详见 `lessons.md`。
