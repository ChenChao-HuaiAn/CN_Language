...
> **交接原因**：2026-08-14 会话结束（**reg_alloc -O2 崩溃缺陷已修复 ✅**——新 Win10 环境 E2E 21/28 崩溃（0xC0000005），根因：x64 prologue `sub rsp, frameSize`（已16对齐）之后 push 被调用者保存寄存器，**奇数个 push 破坏栈 16 字节对齐**，call 前 rsp≡8 (mod 16) 违反 Win x64 ABI，被调方 movaps 崩溃。修复：`emitPrologue` 在 `calleeSavedRegs_` 为奇数时 `frameSize += 8`（满足 `F+8N≡0 mod 16`），最小化改动 +3 行 + 回归单测。单测 965/965、E2E 28/28、-O0/-O2/-O3 三级别 84 组合输出一致、编译零警告，git commit 94fcda9 已推送 gitcode develop）。
>
> **前序里程碑**：阶段3「OOP 与错误处理」（901/901 单测、28/28 E2E）；阶段A「Linux ARM64」（918/918 单测、28/28 E2E）；阶段B 优化补全（949/949 单测、28/28 E2E）；阶段C 寄存器分配/调试信息（964/964 单测、28/28 E2E）；**reg_alloc -O2 崩溃缺陷修复（965/965 单测、28/28 E2E，本交接）**。
>
> **⚠️ 本次新增踩坑教训（务必注意）**：① 凡 prologue 在 `sub rsp, frameSize` **之后** push 被调用者保存寄存器（或任何 8 字节压栈），栈帧大小必须计入 push 字节数——正确公式 **`F + 8N ≡ 0 (mod 16)`**，不能只对 frameSize 单独 16 对齐（N 为奇数时必须 F+8）。② 跨环境（GCC→MSVC、工具链升级）后必须重跑全量 E2E 验证 -O2 与 -O0 一致性；③ 寄存器分配器集成后须验证函数体内 `call` 前 `rsp ≡ 0 (mod 16)`。详见 `lessons.md` 权重 20.8 条目。
...
| 指标 | 数值 |
|------|------|
| 单元测试 | **964/964**（阶段B +31：SSA 7/LICM 6/强度削减 8/内联 6/TCO 6；阶段C +14：reg_alloc 9/debug_info 5） |
