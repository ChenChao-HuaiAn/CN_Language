...
### 一、本轮交付（🏃 311-a 开发轮=306-b·全表空闲依 §8 动态认领·基线 3124d25·2026-09-17）：**波次2 v2 侧同构销项——T3/T4/T21 三案全链闭环（宿主+v2 双侧）**

**三案 v2 侧实施**（对齐宿主 306-a·方案甲已批）：

1. **T21 标识符区段化**：词法分析.cn——是汉字码点（CJK 统一表意/扩展A/B 三区段）+位置码点（原地 UTF-8 解码）+序列长+是标识符起始（**主分发与收集循环单点共用**·306-a OOM 口径分叉教训）+区段外非 ASCII 按序列整体跳过诊断（词法错误加一 进聚合中止链）+**BOM 显式跳过**（收尾事故修复：收紧未枚举旧放行面——BOM 被拒→加 EF BB BF 跳过·对齐宿主「BOM 容忍=口径」）。探针 t21a 全角Ａ拒/t21b 扩展A 㐀 过+**416_v2 转正**。
2. **T3 递归结构体拒绝**：IR布局.cn=未登记引用收集四函数+语法分析.cn=字段收集（排除清单两轮修正：+字符串/用户类+节点附加指针层）+主.cn=聚合守卫——**v2 模型等价完备解**（解析期登记模型下：任何环必产生未登记引用+环成员必登记=同源捕获·合法程序零 miss 零误报）。探针 t3a/b 拒+**t3c 指针递归放行**（首版误拒=类型节点基名+附加形态）+**414_v2 转正**（间接环）。
3. **T4 字符串下标越界**：生成表达式求值.cn=发射字符串越界检查（__cn_str_len 运行时长度+错误码 2 与 94-a 数组防线对称·字符* 视图走指针路径不误伤）——**415_v2 转正**（界内 97/99+越界「运行时错误(错误码2)」·退出码 1=runtime std::exit(1) 既有行为）。

**验收链**：全量 E2E **434=433/0/1**（唯一败=406=T43 已立案待裁决·非本轮写集）+单测 1353/1353+v2 用例族 88/88+78/79 自举链 PASS（fix_p≡fix_s）+宿主 src/ 零改动。

**收尾两事故（同轮根治+lessons 补录）**：①全量首跑 411/413 宿主不拒+05/08/13 基线失真=**陈旧 cn.exe**（rebase 进 ec3e0a2 后未重建——教训第二形态：跨基线合并必须重建）；②05/08/13 v2 侧 BOM 拒绝（T21 收紧未枚举旧放行面）——分词入口 BOM 跳过根治。

### 二、下一轮（按序）

1. **T43/T44 修复后 win 复验**（待用户裁决方案甲/乙/丙三合一·304-a/306-a 已广播）。
2. 清零波次 3~8（plans/021 §3-A 波次行）·T44 x64l/arm64 复验归深度机/单位机。
3. M3 采样续（/goal 不停）。

### 三、坑与边界

- v2 树三坑（lessons 已记）：向量.追加 返回 结果<空类型,整32>；指针字段类型节点=基名+附加指针层；runtime std::exit(1) 恒定（「错误码 N」仅打印文本）。
- **跨基线 rebase 后必须重建 cn.exe**（本轮事故）；收紧类改造先查面台账口径行（BOM 容忍）。
- v2p 手动跑在项目根（隔离 cwd 需 stdlib junction）；探针资产 target/p305a/+probe_*.cn（收尾清理）。

## 深度机 linux-x86_64 节## 深度机 linux-x86_64 节## 深度机 linux-x86_64 节## 深度机 linux-x86_64 节
