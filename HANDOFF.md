...
**最近交接**：2026-09-16——**239-a 两项特性落地（用户批准「编译期常量 if+DCE 调试开关」「货舱 [特性] 声明制」）**：plans/001 新增 §3.8 内建常量 调试模式（--发布/--release 旗标）+§4.7 特性声明制；genIf 常量直取（asm 字节级死分支消除实证）；E2E 309/310；§3-A 增 A10（M1 冲刺）。M1~M8 长线按 plans/026 依赖图滚动中。

### 一、本轮做了什么（写给无上下文的新会话）

用户指令：两项特性落地后立即执行+分多轮完成 plans/026 M1~M8。本轮=特性落地轮：

1. **调试模式（§3.8）**：内建布尔常量——默认真、`--发布`/`--release`（ASCII 别名防 GBK argv 乱码，同 --验证-ir 先例）假；语义层 registerGlobalConstsAndStatics 尾部注入 crate 常量表（用户同名定义优先）+checkConstIdentifier/IR 常量标识符补布尔推断（纯值返回不发指令）+**genIf 常量条件直取**（genIf 加 preCond 可选参+genIfConst 递归链判定——**常规路径零变化**保锚定链）；实测：双配置输出正确+asm 字节级互斥（死分支字符串不生成=与条件编译产物等价零开销）+普通如果两分支全检查。
2. **货舱 [特性]（§4.7 特性小节）**：`启用 = ["网络","压缩"]` TOML 数组解析（parseQuotedArray）→特性并入注入宏集合（-D 同通道）——启用命中/未启用裁剪实测；声明制=宏名单一事实来源（登记制收紧另轮）。
3. **意外事件**：两非写集 src 文件（cn_main.cpp/semantic.hpp）被外部清空为 0 字节——git 恢复无损，lessons 218~219（收工前 git status 全量核对写集纪律）。
4. **A10 立项**（plans/021 §3-A）：M1 完成度冲刺=A9 续波（73 格）+规范冻结清单+M1 三判据。
5. 规范：plans/001 §3.8/§4.7+spec 01a 同步；覆盖矩阵 61 单元 60.1%。

### 二、验证链

构建零警告 ✓；双配置探针（rc=0+asm 字节级）✓；货舱特性探针 ✓；覆盖门禁 --ci（61 单元·60.1%）✓；全量 ci.ps1（338 用例口径）结果见 plans/021 文首戳收工数字。

### 三、下一步（新会话按序·M1~M8 长线）

1. **A10 M1 冲刺**（新立项）：A9 续波补格（73 格→--strict 100%）+规范冻结残余销项。
2. **M1 后依 plans/026 §六依赖图**：M3 需支柱二 CN-Smith（验证轮档期建生成器）→M4 需 F1-26+D8→M5 需 C3 波 4/7/8+ASan 扩面；M8 设施层 L1~L3 可并行。
3. 登记制收紧（§4.7 未声明宏名警告→错误）独立轮（契约变更）。

### 四、诚实边界

- `--发布` 中文旗标在 Git Bash argv 经 GBK 乱码不可用（ASCII 别名 `--release` 可用）——cmd/PowerShell 下中文旗标待验证。
- 309/310 固化默认配置面；`--release` 配置面=手工实测回填（asm 互斥证据在收工提交信息），runner 无每用例编译参数机制（扩展另轮）。
- 两分支全检查的负例面（调试模式分支内语义错误必报）未单独入库用例（探针已验证机制=普通如果语义；随 A9 负例波补）。
