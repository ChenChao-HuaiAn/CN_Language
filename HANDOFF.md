...
5. **E2E 279_用户类字段深拷 转正**（213 同款「残留:0」口径：形一~形四+自赋值+深拷独立性）。

### 二、验证链（行为改动轮口径）

零警告构建 + 单测 **1317/1317** + 探针全家族 ×3（userclass 残留 2→**全 0**）+ 116 ×3 PASS +
全量 E2E **307 用例 307 过/0 败/0 跳过** + **锚定链两层判定**：层 1 v2p asm +2147 行（v2 树命中实锤）；
