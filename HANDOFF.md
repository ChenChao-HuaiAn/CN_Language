...
**最近交接**：2026-09-16——**243-a D17 防线/D18 观察期 + C 类 4 格**：D17 重新归因（主 返回 结果 的 sret ABI 错位·与 ? 无关）+编译期防线；D18 警告观察期（fallthrough 教训·消融定位）；C 类 4 格（350~353）——覆盖率 **84.7%**；全量 CI 全绿（E2E 375=375/0/0）。

### 一、本轮做了什么（写给无上下文的新会话）

1. **D17 重新归因+防线**：三分差分实验（主结果+?/主结果无?/普通函数+?）实锤真凶=「主 返回 结果」的 **sret ABI 错位**（runtime `extern "C" int cn_main()` 期望整32，聚合返回走隐藏指针→调用约定错位→退出段错误——与 ? 无关）。退出码语义（正常→值/错误→错误码）属**语言语义新增待审批**——本轮落编译期防线（主 返回非整32→诊断「暂不支持」·344D 锚定）把必然崩溃挡在编译期。根治待语义裁决。
2. **D18 警告观察期**：checkInstanceMethodCall 补常量上下文警告（`自身.非常量方法`）。**重大教训（消融定位）**：首版检查块 `return true` 跳过了 retOwnedString/借用登记等正常调用语义→220/64/221-D2 三个映射集合消费者运行段错误——改为**警告后 fallthrough**。stdlib 映射集合存量违约→**D19 登记（stdlib 常量性标注迁移）**；硬错误随观察期收口。
3. **C 类 4 格**：350 浮点隐式转整拒绝（3.3 负例）/351 联合体嵌套结构体边界——不安全 函数 内（157-a 口径）/352 混合转换边界（3.7）/353 调试模式字符串上下文拒绝（3.8 负例）——覆盖率 **84.7%**（155/183·三态全 38）。
4. **补登**：D17/D18 系 242-a 遗漏登记（登记脚本中断未写入）——本轮补登并修正。

### 二、验证链（全绿）

全量 ci.ps1 v5：单测 **1327/1327**+E2E **375=375/0/0**（370 旧零回归+5 新·348 撤）；--ci PASS（84.7%）；消融实验定位 220 崩因；负例三连复验保持（344D/338/339）。

### 三、下一步（新会话按序）

1. **D17 退出码语义呈报**（待用户批准后根治 wrapper 方案）；**D19 stdlib 常量性迁移**（D18 升硬错误前置）。
2. **B3 口径裁决**（仍待用户）→A10 第四波收官路径。
3. M1 收官后：CN-Smith（M3）→F1-26+基准（M4）→安全矩阵+ASan（M5）→M6/M8。

### 四、诚实边界

- D17 防线=「编译期拒绝」非最终形态——根治（wrapper 生成）待退出码语义批准。
- D18 观察期警告在 stdlib 迁移（D19）前持续输出。
