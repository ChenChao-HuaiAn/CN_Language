...
**遗留/后续**：~~v2 自举编译器的 linux-x86_64 代码生成（代码生成X64Linux.cn）=后续专项~~（✅ 已由 plans/017 专项完成并三级锚定收口，2026-09-05~07 第十九~二十七轮）；~~win x64 第4路判定与形态 A 的交互建议家机复核一轮~~（✅ 第八节 2026-09-05 家机 win-x64 呈报复核方案A 当轮根治闭环）。
...
## 十、被调方栈参数锚定基缺陷根治（2026-09-06 深度系统 x86_64，plans/017 T5 冒烟揪出）

**现象**：v2p（linux-x86_64 构建）编译探针的 IR 中实参位号/被调符号/类型字段被**代码地址污染**（如调用实参位号=0x43EAB6），产物 asm `call` 空操作数、标签重复——无法汇编。12 组件 HEAD 版 v2p 与 7650713 v2 组件同样坏（对照实验）——与本轮新发射器无关。

**根因**（gdb watch + objdump 三方对表闭环）：`emitParamSetup` 被调方栈参数锚定基恒 `16+stackIdx*8`——但 needHiddenRet（返回结构体/i128）时 prologue `push rbx`（保存隐藏返回指针于 [rbp+8]）使栈参数实际位于 **[rbp+24+8k]**：被调方读 [rbp+16] 读到的是**返回地址**（污染值全为返回地址的铁证）、[rbp+24] 读到第 7 参（错位承接）。触发条件=「返回结构体 + 整型参数≥7」（第 7 参起走栈）——v2 组件 建IR指令（7 参+返回 IR指令 结构体 56 字节）完美踩中；E2E 153 仅 6 参全走寄存器故未触发；单位机历史绿=linux-arm64 宿主构建（**同源码不同宿主后端构建的 v2p 行为分叉**——arm64 侧锚已按 needHiddenRet 补偿，见 arm64_codegen.cpp:685 注释「栈参数偏移 = 16 + 16*needHiddenRet」；win 无 push rbx 锚恒 48 不受影响）。

**修复**：`stackAnchor = 16 + 8*paramOffset`（emitParamSetup 三处消费）——与 arm64 侧同构。

**为何历史门禁未暴露**：v2p 产物级验证在本机历史空白（asm 只验产出未汇编+v2 闭环 23 例平台跳过+宿主 E2E 无「返回结构体+≥7 参数」形态）。

**验证**：E2E **154_linuxx64_隐藏返回指针栈参数锚定**（返回结构体+8 整型参数，第 7/8 栈参断言）**红→绿闭环**（回滚版 420598354321=污染形态/修复版 87654321）+ 单测 HiddenRetStackParamAnchor24（锚 24 断言+旧锚 16 形态排除）+ v2p 重建后全部探针 IR 干净（位号 0/1、符号 ID 恢复）。

**连带揪出并根治（v2 侧，详见 plans/017 第八节）**：静态赋值存储丢失/限定名内置 结果 接收 ABI 错配/返回含容器字段结构体所有权悬垂/跨文件同名函数符号冲突（生成函数→生成函数W）。
