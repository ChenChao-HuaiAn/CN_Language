...
**设计说明**：
- CN语言简化了C语言的语法，统一使用 `.` 访问成员
- 编译器在生成C代码时自动判断类型：
  - 对象类型 → 生成 `obj.member`
  - 指针类型 → 生成 `ptr->member`
- `->` 运算符作为可选项保留，用于显式表达指针解引用语义

**注意**：虽然 `->` 语法也被支持，但推荐统一使用 `.` 以保持代码风格一致。
...
类型枚举 `CnTypeKind` 定义在 [`semantics.h:23-52`](include/cnlang/frontend/semantics.h:23)，包含基础类型、复合类型和扩展数字类型。

**类型种类列表**：

| C枚举值 | CN语言枚举值 | 说明 |
|---------|-------------|------|
| `CN_TYPE_VOID` | `类型_空` | 无类型 |
| `CN_TYPE_INT` | `类型_整数` | 整型（默认64位） |
| `CN_TYPE_FLOAT` | `类型_小数` | 浮点型（默认64位） |
| `CN_TYPE_CHAR` | `类型_字符` | 字符型 |
| `CN_TYPE_BOOL` | `类型_布尔` | 布尔型 |
| `CN_TYPE_STRING` | `类型_字符串` | 字符串型 |
| `CN_TYPE_POINTER` | `类型_指针` | 指针类型 |
| `CN_TYPE_ARRAY` | `类型_数组` | 数组类型 |
| `CN_TYPE_STRUCT` | `类型_结构体` | 结构体类型 |
| `CN_TYPE_ENUM` | `类型_枚举` | 枚举类型 |
| `CN_TYPE_CLASS` | `类型_类` | 类类型 |
| `CN_TYPE_INTERFACE` | `类型_接口` | 接口类型 |
| `CN_TYPE_FUNCTION` | `类型_函数` | 函数类型 |
| `CN_TYPE_PARAM` | `类型_参数` | 类型参数（泛型） |
| `CN_TYPE_MEMORY_ADDRESS` | `类型_内存地址` | 内存地址类型 |
| `CN_TYPE_INT32` | `类型_32位整数` | 32位有符号整数 |
| `CN_TYPE_INT64` | `类型_64位整数` | 64位有符号整数 |
| `CN_TYPE_UINT32` | `类型_32位无符号` | 32位无符号整数 |
| `CN_TYPE_UINT64` | `类型_64位无符号` | 64位无符号整数 |
| `CN_TYPE_UINT64_LL` | `类型_64位无符号LL` | 64位无符号整数(ULL) |
| `CN_TYPE_FLOAT32` | `类型_单精度小数` | 单精度浮点数 |
| `CN_TYPE_FLOAT64` | `类型_双精度小数` | 双精度浮点数 |
| `CN_TYPE_SELF` | `类型_自身` | 自身类型 |
| `CN_TYPE_UNKNOWN` | `类型_未定义` | 未知类型 |
