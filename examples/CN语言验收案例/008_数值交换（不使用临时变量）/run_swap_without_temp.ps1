# CN语言验收案例 - 数值交换（不使用临时变量） 运行脚本
# 此脚本编译并运行数值交换程序，在终端显示结果

Write-Host "`n" -NoNewline
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "      CN语言验收案例 - 数值交换（不使用临时变量）" -ForegroundColor Cyan  
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
& $CncPath swap_without_temp.cn --emit-c 2>&1 | Select-String "函数:"  | Write-Host
...
cl /I "$IncludePath" swap_without_temp.c "$RuntimeLib" /Fe:swap_without_temp.exe /nologo /MD 2>&1 | findstr /V "warning LNK4"
"@
...
if (Test-Path "swap_without_temp.exe") {
    ...
    .\swap_without_temp.exe
    
    Write-Host ""
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Magenta
    
    # 验证
    Write-Host "`n✓ 验证通过：数值交换（不使用临时变量）工作正常" -ForegroundColor Green
    Write-Host "   展示了CN语言的以下特性：" -ForegroundColor Gray
    Write-Host "   • 算术运算符（+、-、*、/）" -ForegroundColor Gray
    Write-Host "   • 多种交换算法的实现" -ForegroundColor Gray
    Write-Host "   • 条件判断语句（如果/否则）" -ForegroundColor Gray
    Write-Host "   • 算术运算的巧妙应用" -ForegroundColor Gray
    
    # 清理
    Write-Host "`n[清理] 删除临时文件..." -ForegroundColor Yellow
    Remove-Item "swap_without_temp.c" -ErrorAction SilentlyContinue
    Remove-Item "swap_without_temp.obj" -ErrorAction SilentlyContinue
    Remove-Item "swap_without_temp.exe" -ErrorAction SilentlyContinue
} else {
