# CN语言验收案例 - 温度转换 运行脚本
# 此脚本编译并运行温度转换程序，在终端显示结果

Write-Host "`n" -NoNewline
Write-Host "════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "   CN语言验收案例 - 温度转换（华氏转摄氏）" -ForegroundColor Cyan  
Write-Host "════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
& $CncPath temperature_convert.cn --emit-c 2>&1 | Select-String "函数:"  | Write-Host
...
cl /I "$IncludePath" temperature_convert.c "$RuntimeLib" /Fe:temperature_convert.exe /nologo /MD 2>&1 | findstr /V "warning LNK4"
"@
...
if (Test-Path "temperature_convert.exe") {
    ...
    $output = & ".\temperature_convert.exe" 2>&1
    Write-Host $output -ForegroundColor White
    
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Magenta
    
    # 验证输出 - 100°F = 37°C (整数除法)
    if ($output -match "37") {
        Write-Host "`n✓ 验证通过：温度转换正确 (100°F ≈ 37°C)" -ForegroundColor Green
        Write-Host "   说明：当前使用整数运算，(100-32)*5/9 = 37（整数除法）" -ForegroundColor Gray
    } else {
        Write-Host "`n✗ 验证失败：转换结果不正确" -ForegroundColor Red
    }
    
    # 清理
    Write-Host "`n[清理] 删除临时文件..." -ForegroundColor Yellow
    Remove-Item "temperature_convert.c" -ErrorAction SilentlyContinue
    Remove-Item "temperature_convert.obj" -ErrorAction SilentlyContinue
    Remove-Item "temperature_convert.exe" -ErrorAction SilentlyContinue
} else {
