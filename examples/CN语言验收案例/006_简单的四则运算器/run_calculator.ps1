# CN语言验收案例 - 简单的四则运算器 运行脚本
# 此脚本编译并运行四则运算器程序，在终端显示结果

Write-Host "`n" -NoNewline
Write-Host "════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "   CN语言验收案例 - 简单的四则运算器" -ForegroundColor Cyan  
Write-Host "════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
& $CncPath calculator.cn --emit-c 2>&1 | Select-String "函数:"  | Write-Host
...
cl /I "$IncludePath" calculator.c "$RuntimeLib" /Fe:calculator.exe /nologo /MD 2>&1 | findstr /V "warning LNK4"
"@
...
if (Test-Path "calculator.exe") {
    ...
    # 验证
    Write-Host "`n✓ 验证通过：四则运算器工作正常" -ForegroundColor Green
    Write-Host "   展示了CN语言的以下特性：" -ForegroundColor Gray
    Write-Host "   • 加法运算符 +" -ForegroundColor Gray
    Write-Host "   • 减法运算符 -" -ForegroundColor Gray
    Write-Host "   • 乘法运算符 *" -ForegroundColor Gray
    Write-Host "   • 除法运算符 /" -ForegroundColor Gray
    Write-Host "   • 取模运算符 %" -ForegroundColor Gray
    Write-Host "   • 完整的算术表达式计算" -ForegroundColor Gray
    
    # 清理
    Write-Host "`n[清理] 删除临时文件..." -ForegroundColor Yellow
    Remove-Item "calculator.c" -ErrorAction SilentlyContinue
    Remove-Item "calculator.obj" -ErrorAction SilentlyContinue
    Remove-Item "calculator.exe" -ErrorAction SilentlyContinue
} else {
