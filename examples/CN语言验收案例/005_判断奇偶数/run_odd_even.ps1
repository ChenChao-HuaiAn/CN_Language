# CN语言验收案例 - 判断奇偶数 运行脚本
# 此脚本编译并运行奇偶数判断程序，在终端显示结果

Write-Host "`n" -NoNewline
Write-Host "════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "   CN语言验收案例 - 判断奇偶数" -ForegroundColor Cyan  
Write-Host "════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
& $CncPath check_odd_even.cn --emit-c 2>&1 | Select-String "函数:"  | Write-Host
...
cl /I "$IncludePath" check_odd_even.c "$RuntimeLib" /Fe:check_odd_even.exe /nologo /MD 2>&1 | findstr /V "warning LNK4"
"@
...
if (Test-Path "check_odd_even.exe") {
    ...
    # 验证
    Write-Host "`n✓ 验证通过：条件判断语句工作正常" -ForegroundColor Green
    ...
    Write-Host "   • 比较运算符 ==" -ForegroundColor Gray
    
    # 清理
    Write-Host "`n[清理] 删除临时文件..." -ForegroundColor Yellow
    Remove-Item "check_odd_even.c" -ErrorAction SilentlyContinue
    Remove-Item "check_odd_even.obj" -ErrorAction SilentlyContinue
    Remove-Item "check_odd_even.exe" -ErrorAction SilentlyContinue
} else {
