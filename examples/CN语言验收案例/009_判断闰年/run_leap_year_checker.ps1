# CN语言验收案例 - 判断闰年 运行脚本
# 此脚本编译并运行判断闰年程序，在终端显示结果

Write-Host "`n" -NoNewline
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "           CN语言验收案例 - 判断闰年" -ForegroundColor Cyan  
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
& $CncPath leap_year_checker.cn --emit-c 2>&1 | Select-String "函数:"  | Write-Host
...
cl /I "$IncludePath" leap_year_checker.c "$RuntimeLib" /Fe:leap_year_checker.exe /nologo /MD 2>&1 | findstr /V "warning LNK4"
"@
...
if (Test-Path "leap_year_checker.exe") {
    ...
    # 验证
    Write-Host "`n✓ 验证通过：判断闰年程序工作正常" -ForegroundColor Green
    Write-Host "   展示了CN语言的以下特性：" -ForegroundColor Gray
    Write-Host "   • 算术运算符（%, /, *, +, -）" -ForegroundColor Gray
    Write-Host "   • 布尔类型和真假值（真/假）" -ForegroundColor Gray
    Write-Host "   • 条件判断语句（如果/否则）" -ForegroundColor Gray
    Write-Host "   • 函数定义与调用" -ForegroundColor Gray
    Write-Host "   • 返回语句和布尔返回值" -ForegroundColor Gray
    Write-Host "   • 多层嵌套条件逻辑" -ForegroundColor Gray
    
    # 清理
    Write-Host "`n[清理] 删除临时文件..." -ForegroundColor Yellow
    Remove-Item "leap_year_checker.c" -ErrorAction SilentlyContinue
    Remove-Item "leap_year_checker.obj" -ErrorAction SilentlyContinue
    Remove-Item "leap_year_checker.exe" -ErrorAction SilentlyContinue
} else {
