...
const char* kVectorDef = R"CN(泛型 <类型 T>
类 向量 {
私有:
    T* 数据;
公开:
...
const char* kMapDef = R"CN(泛型 <类型 K, 类型 V>
类 映射 {
私有:
    V* 值表;
公开:
...
TEST(BorrowViewCheckTest, RejectElemAfterRemove) {
    auto r = analyzeSource(std::string(kVectorDef) + R"CN(不安全 函数 坏() -> 整32 {
    向量<字符串> 表 = 向量<字符串>();
    字符串 借出 = 表.元素(0);
    结果<空类型, 整32> 删 = 表.删除(0);
    返回 整32(字符串长度(借出));
...
TEST(BorrowViewCheckTest, RejectMapGetAfterRemove) {
    auto r = analyzeSource(std::string(kMapDef) + R"CN(不安全 函数 坏() -> 整32 {
...
TEST(BorrowViewCheckTest, AllowImmediateConsume) {
    auto r = analyzeSource(std::string(kVectorDef) + R"CN(不安全 函数 好() -> 整32 {
    向量<字符串> 表 = 向量<字符串>();
...
const char* kShelfClassDef = R"CN(类 货架 {
公开:
    向量<字符串> 表;
    不安全 函数 货架() {
        表 = 向量<字符串>();
...
TEST(BorrowViewCheckTest, RejectMemberChainBorrow) {
    auto r = analyzeSource(std::string(kVectorDef) + std::string(kShelfClassDef) +
        R"CN(不安全 函数 坏() -> 整32 {
    货架 架 = 货架();
    字符串 借出 = 架.表.元素(0);
...
TEST(BorrowViewCheckTest, RejectContainerAndViewAsArgs) {
    auto r = analyzeSource(std::string(kVectorDef) + R"CN(
