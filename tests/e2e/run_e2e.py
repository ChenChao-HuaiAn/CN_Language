...
#   给足 32768（物理 128GB·真实验证通过即可）——611-a 同源第三次扩展。
# 611-a：4096→8192——v2 树 def-init 同构（+330 行）后 78/79 fix_p 全树编译
...
#   （用户两次质询）；其执行链=v1 自举组件（CN语言编译器/·因字符串行 IR 内存失控被推倒）
#   → 同族 70~77 全套 + 89（引用 v1 树）一并纳入保护面（B9 族面扩散口径）。
#   超限即杀 + 判失败（防 OOM 卡死拖垮机器）。
# 601-a（v1 收口·573 裁决甲）：70~77/89 v1 化石用例已下线——内存保护名单
#   仅保留 v2 自举链重载面（全树编译 OOM 防线·与用例共存亡）。
...
def 并行用例任务(项):
    ...
    内存上限随任务项传递（605-a）：win spawn 模式 worker 重新 import 本模块，
    主进程 main() 对 内存上限MB默认 的 --max-mem-mb 覆盖不会进入 worker——
    CLI 覆盖值必须显式传参，worker 侧还原为模块全局后方才对 752/819 等消费点生效。"""
    用例目录, 编译器路径, 输出目录, 详细, 目标平台, 内存上限 = 项
    global 内存上限MB默认
    内存上限MB默认 = 内存上限
...
def 执行单个用例(编译器路径: pathlib.Path, 用例目录: pathlib.Path,
    ...
    if v2配置路径.exists():
        ...
        供给源们 = v2配置["供给源们"]
        # 325-a 补漏（261-a 加「断言输出」配置键时本调用点漏传——默认 False 恒走
        #   锚行模式；85 个存量 v2闭环.txt 均未写该键=行为零变更，新用例可用）
        return 执行v2闭环(编译器路径, 用例目录, 输出目录, 详细, 目标平台, 源文件名们, 预期退出码,
                        链接v2pobj, 供给源们, v2配置["断言输出"])
...
def 执行双编译对照(编译器路径: pathlib.Path, 用例目录: pathlib.Path,
    ...
    #   全值比对；dual 通道原恒 %256，对退出码>255 用例（123=21333 首例）win
    #   必假红。linux 保持 POSIX 8 位截断口径（与 执行v2闭环Linux 一致）。
    if 目标平台 == "win-x64":
        宿主期望 = 预期退出码
    else:
        宿主期望 = 预期退出码 % 256
    if 宿主码 != 宿主期望:
        return "失败", (f"{编号}-D2 宿主侧退出码={宿主码}（期望 {宿主期望} = {预期退出码}"
                        f"{'）' if 目标平台 == 'win-x64' else ' % 256）'}"
