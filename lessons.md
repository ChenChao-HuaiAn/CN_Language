...
- [2026-08-15 04:30] **问题类型**: 设计缺陷（权重 12.0）⚠️ 已规避（记录，待 Debug 子任务修复）
  ...
  - **解决**: 规避——每个函数拆到栈帧 <8KB（少打印、少局部变量、容器实例单函数仅放 1 个，如 向量演示/集合演示/映射演示 独立函数）；数据跨函数用 数组指针参数（整64*/字符串*）；结构体按值返回正常（统计结果 40 字节已验证）。**根治方向**：codegen 栈帧 >4KB 时发射 `call __chkstk`（MSVC 惯例，__chkstk 按 4KB 页提交）或按页分次 sub
...
- [2026-08-15 02:40] **问题类型**: 设计缺陷（权重 11.0）⚠️ 已规避（记录，待 Debug 子任务修复）
...
- [2026-08-15 02:50] **问题类型**: 逻辑错误（权重 12.0）⚠️ 已规避（记录，待 Debug 子任务修复）
...
- [2026-08-15 02:55] **问题类型**: 设计缺陷（权重 10.5）⚠️ 已规避（记录，待 Debug 子任务修复）
...
- [2026-08-14 18:30] **问题类型**: 设计缺陷（权重 12.0）⚠️ 已规避（记录，待 Debug 子任务修复）
