...
- **大小与安全**：`--cfi` 开启接口间接调用控制流完整性校验（分派目标 ∈ 已知实现表，默认关保性能；见 plans/008）
...
- E2E：**103/103**（2026-08-18 快照口径；最新门禁口径见 `HANDOFF.md` 与 `plans/021`——含 78_v2 自举链构建 / 79_v2 自举闭环（v2 锚定链），2026-09-14 自 v1 78_chain_build / 79_bootstrap_closed_loop 迁移）
- 编译警告：0（MSVC /W4 /WX）
...
- ~~引用变量与引用返回暂不支持~~（已支持，e2e 94/102）——引用变量（`整32& r = x`）、引用返回（`函数 获取() -> 整32&`，返回被引用左值地址，e2e 102；返回局部变量地址报错 e2e 103）
