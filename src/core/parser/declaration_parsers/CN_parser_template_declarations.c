/**
 * @file CN_parser_template_declarations.c
 * @brief CN_Language 模板声明解析模块
 * 
 * 模板声明解析模块，负责解析模板声明。
 ...
 * @brief 解析模板声明（内部实现）
 */
static Stru_AstNode_t* parse_template_declaration_impl(Stru_ParserInterface_t* interface);

/**
 * @brief 解析模板参数列表
 */
static Stru_DynamicArray_t* parse_template_parameter_list(Stru_ParserInterface_t* interface);
...
{
    return parse_template_declaration_impl(interface);
}
...
static Stru_AstNode_t* parse_template_declaration_impl(Stru_ParserInterface_t* interface)
{
    ...
    // 检查模板关键字
    if (state->current_token->type != Eum_TOKEN_KEYWORD_TEMPLATE)
    {
        // 报告错误
        F_report_unexpected_token_error(interface, state->current_token, "模板");
        return NULL;
    }
    
    // 保存模板关键字令牌
    Stru_Token_t* template_token = state->current_token;
    ...
    // 保存模板名称令牌
    Stru_Token_t* template_name_token = state->current_token;
    const char* template_name = template_name_token->lexeme;
    
    // 消耗模板名称
    F_advance_token(state);
    
    // 解析模板参数列表（可选）
    Stru_DynamicArray_t* template_params = NULL;
    ...
    if (state->current_token != NULL && state->current_token->type == Eum_TOKEN_OPERATOR_LESS)
    {
        ...
        // 解析模板参数列表
        template_params = parse_template_parameter_list(interface);
        ...
        if (state->current_token == NULL || state->current_token->type != Eum_TOKEN_OPERATOR_GREATER)
        {
            ...
            if (template_params != NULL)
            {
                F_destroy_dynamic_array(template_params);
            }
            return NULL;
        }
    ...
    // 创建模板声明节点
    // 注意：暂时使用Eum_AST_STRUCT_DECL作为模板声明节点类型
    // 后续可以添加专门的Eum_AST_TEMPLATE_DECL节点类型
    Stru_AstNode_t* template_node = F_parser_create_ast_node(interface, Eum_AST_STRUCT_DECL, template_token);
    
    // 设置模板名称
    F_parser_set_node_attribute(interface, template_node, "name", (void*)template_name);
    
    // 设置模板参数列表作为属性
    if (template_params != NULL)
    {
        F_parser_set_node_attribute(interface, template_node, "template_params", template_params);
    }
    
    // 设置成员列表作为属性
    F_parser_set_node_attribute(interface, template_node, "members", members);
    
    // 设置节点类型为模板
    F_parser_set_node_attribute(interface, template_node, "declaration_type", (void*)"template");
