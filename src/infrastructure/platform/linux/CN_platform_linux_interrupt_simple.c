/******************************************************************************
 * 文件名: CN_platform_linux_interrupt_simple.c
 * 功能: CN_Language Linux平台中断控制器简单实现
 * 描述: Linux平台中断控制器基础实现，提供基本的中断处理功能
 * 作者: CN_Language开发团队
 * 创建日期: 2026年1月
 * 修改历史:
 *  2026-01-03: 创建文件，实现Linux平台中断控制器基础功能
 * 版权: MIT许可证
 ******************************************************************************/
...
typedef struct {
    ...
    Stru_CN_LinuxInterruptHandlerInfo_t handlers[CN_LINUX_MAX_INTERRUPT_HANDLERS];
} LinuxInterruptControllerState;

static LinuxInterruptControllerState g_controller_state = {
...
static bool linux_interrupt_controller_get_info(Stru_CN_InterruptControllerInfo_t* info)
{
    ...
    strncpy(info->name, "Linux Signal Controller", sizeof(info->name) - 1);
                                                       ...
                                                       const Stru_CN_InterruptDescriptor_t* desc)
{
    ...
    // 检查中断号是否在有效范围内
    if (irq < 1001 || irq > 1064) {
        return false;
    }
    
    // 查找现有的处理程序或空闲槽位
    Stru_CN_LinuxInterruptHandlerInfo_t* handler_info = linux_find_handler_by_interrupt(irq);
    if (!handler_info) {
    ...
    if (desc && desc->description[0] != '\0') {
        strncpy(handler_info->name, desc->description, CN_LINUX_MAX_INTERRUPT_NAME - 1);
...
{
    // 简单实现：等同于禁用中断
    return linux_interrupt_controller_disable_interrupt(irq);
}
...
{
    // 简单实现：等同于启用中断
    return linux_interrupt_controller_enable_interrupt(irq);
}
...
{
    // 简单实现：检查是否启用
    return !linux_interrupt_controller_is_interrupt_enabled(irq);
}
...
{
    return &g_linux_interrupt_controller_interface;
}
...
{
    return linux_interrupt_controller_initialize();
}
...
{
    linux_interrupt_controller_cleanup();
}
...
{
    return linux_interrupt_controller_is_initialized();
}
