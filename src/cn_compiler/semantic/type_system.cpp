...
//   3. 整型宽化（2026-09-10 注释归真——原注释「仅同符号向更高秩/同秩跨符号
//      不允许」与实现不符，plans/014 观察项①收口；实现自始即如下三条）：
//      a. 同符号宽化（整8->整32、正8->正64）允许
//      b. 同秩跨符号（正32<->整32）允许隐式：位模式一致，仅解释不同
//         （C++ 语义；打印(整64) 传 正64 等场景需要）
//      c. 异秩跨符号宽化（整32->正64）允许（下方 rankFrom<rankTo 不查符号）
//      ⚠ 挂账（plans/014 观察项①+③）：Rust E0308 在赋值初始化/函数传参/
//      返回面拒绝一切变量间跨符号隐式转换（仅字面量推断豁免）；二元运算面
//      已按 §3.7 第五十一轮方案A 严格化（混合符号二元运算禁止），赋值/传参
//      面严格化=三方案呈报待用户裁决，裁决前维持本实现
//   4. 浮点宽化：浮32 -> 浮64
