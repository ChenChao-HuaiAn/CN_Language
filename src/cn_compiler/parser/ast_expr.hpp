...
public:
    ...
    // 588-a（005〔原B-T81〕取地址语义一致化·用户裁决甲 2026-09-21）：语义层引用
    //   形参包装（wrapRefArgs）合成的取地址=true——IR 层按"内部引用机制形态"
    //   发射 AddrOf(变量槽)（A-1 byRef 契约：引用形参槽存被引用左值地址）；
    //   用户显式 &变量=false——类类型发射 Load(槽)（产对象地址·一级语义，
    //   probe_f1 静默垃圾值根治），其余类型仍 AddrOf(槽)。
    bool refWrapAddr = false;
};
