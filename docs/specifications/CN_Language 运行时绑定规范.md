...
| `对象.长度()` | `cn_rt_string_length` 或 `cn_rt_array_length` | 方法风格调用，与函数风格等价 |
...
1. **函数风格**：`长度(对象)` 
   - 示例：`变量 len = 长度(arr);`
   - 示例：`变量 len = 长度("你好");`
   - 参数类型可为数组或字符串，编译器根据类型自动选择相应的运行时函数

2. **方法风格**：`对象.长度()`
   - 示例：`变量 len = arr.长度();`
   - 示例：`变量 len = "你好".长度();`
   - 不接受参数，对象通过成员访问语法提供
   - 内部实现：编译器将其转换为相应的函数风格调用

3. **返回类型**：两种方式都返回 `整数` 类型

4. **类型检查**：
   - 函数风格：参数必须为数组或字符串类型
   - 方法风格：对象必须为数组或字符串类型，不接受额外参数
