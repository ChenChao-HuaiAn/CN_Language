...
## 4.2 内存管理
- [x] **4.2.1 设计内存管理接口**：新建 `include/cnlang/runtime/memory.h` 定义统一内存分配接口。
- [x] **4.2.2 实现基础分配器**：在 `src/runtime/memory/memory.c` 中封装 `cn_rt_malloc/free/realloc`。
- [x] **4.2.3 添加内存分配追踪**：可选调试模式，追踪分配/释放统计。
- [x] **4.2.4 预留自定义分配器钩子**：为 OS 开发场景预留页分配器接口。
- [x] **4.2.5 迁移现有分配调用**：将现有模块中的 `malloc/free` 替换为运行时统一接口。
- [x] **4.2.6 更新 CMakeLists**：将 memory 模块加入构建。
...
## 4.4 与语言特性绑定
- [x] **4.4.1 文档化运行时依赖**：明确哪些语言特性依赖运行时。
- [x] **4.4.2 字符串类型绑定**：确保 C 后端生成正确的运行时调用。
- [x] **4.4.3 数组类型绑定**：确保数组操作使用运行时接口。
- [x] **4.4.4 内置函数映射**：将语言内置函数（如 `长度()`）映射到运行时调用。

## 4.5 测试覆盖
- [x] **4.5.1 创建运行时单元测试目录**：建立 `tests/unit/runtime/`。
- [x] **4.5.2 内存管理单元测试**：测试分配/释放/追踪功能。
- [x] **4.5.3 字符串函数单元测试**：测试所有字符串操作函数。
- [x] **4.5.4 数组函数单元测试**：测试数组分配/越界检查。
- [x] **4.5.5 I/O 函数单元测试**：测试输入输出函数。
- [x] **4.5.6 运行时集成测试**：编写依赖运行时的 `.cn` 示例并验证。
- [x] **4.5.7 更新测试 CMakeLists**：将运行时测试加入构建。
