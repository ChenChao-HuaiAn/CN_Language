...
### 一、整体里程碑与版本策略
- [x] **阶段范围**：完成开发计划中 **阶段 1–7 的"核心目标"**,阶段 8 至少完成"核心语法扩展"的最小子集(指针、结构体、枚举、预处理器等)。
  - ✅ **完成情况**:阶段1-7核心目标已完成并通过验收(参见阶段7最终验收报告);阶段8进度14/15(93%),核心语法扩展(指针、结构体、枚举、函数指针、预处理器)已全部实现。
- [x] **版本区间**:从当前 `0.y.z` 版本迭代到 `0.9.z`,稳定后发布 `1.0.0`(参见版本号规范第 5 章)。
  - ✅ **完成情况**:已设置当前版本为 `0.9.0-beta.1`,符合1.0发布前的版本规划;版本号管理体系已建立(CMakeLists.txt + version.h)。
- [x] **稳定性要求**:1.0 之后对语言与公共 API 提供源兼容性承诺(参见版本号规范第 6 章)。
  - ✅ **完成情况**:版本号规范已明确定义兼容性承诺;阶段7建立了完整的回归测试集,为稳定性提供保障。
- [x] **预发布流程**:至少经历 `1.0.0-beta.N` 与 `1.0.0-rc.N` 阶段,分别用于公开测试与发布候选验证。
  - ✅ **完成情况**:当前版本 `0.9.0-beta.1` 已进入Beta阶段;版本号规范明确定义了预发布流程(alpha -> beta -> rc -> 正式版)。
- [ ] **直接二进制后端目标**:在至少一个主力平台上,CN 编译器可在 **无需外部 C 编译器** 的情况下,直接生成可执行程序或可链接的目标文件。
  - ⚠️ **当前状态**:阶段8已在Linux/WSL2环境实现ELF可执行文件生成,但仍依赖外部C编译器和链接器;直接二进制后端需作为1.x或2.0的后续目标。
  - 📝 **建议**:将此目标调整为"1.0支持C后端作为稳定路径,直接二进制后端作为1.x实验特性,2.0达到生产可用"。
