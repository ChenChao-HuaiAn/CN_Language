...
- **6.16 阶段 6 综合测试清单**
  - [x] 为 REPL、格式化工具、静态检查工具分别整理测试清单，确保：
    - [x] 每个工具都有对应的单元测试和至少一个集成测试场景；
    - [x] 覆盖典型使用路径和主要错误场景。
  - [x] 在 `ctest` 配置中纳入新增加的测试目标，保证一键执行即可验证阶段 6 的主要能力。
  - [x] 创建综合测试清单文档：`docs/implementation-plans/阶段 6：工具链与生态/阶段 6 综合测试清单.md`
  - [x] 为测试添加标签（stage6, repl, format, check, unit, integration）方便分类执行

- **6.17 与里程碑的对应关系**
  - [x] 明确标记哪些测试/用例对应开发计划中的里程碑：
    - [x] REPL：能在命令行快速试验语法和小段程序；
    - [x] 格式化工具：能对样例项目进行统一格式化，结果稳定可复现；
    - [x] 静态检查工具：能在简单场景下发现未使用变量、复杂度超限等问题。
  - [x] 在阶段结束前，对照本 TODO 列表逐项检查完成情况，并在需要时更新 `CN_Language 开发计划.md` 的进度标记。
  - [x] 在综合测试清单文档中添加里程碑对应关系章节（第 9 章）