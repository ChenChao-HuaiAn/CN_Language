...
2. **典型关键字（当前项目常用子集）**：
   ...
   - 变量与声明：`变量`、`常量`；
   - 函数与控制：`函数`、`返回`、`主程序`、`如果`、`否则`、`当`、`循环`、`选择`、`情况`、`默认`、`中断`、`继续`；
...
4. **返回类型约定**：
   ...
   - 如果不显式声明返回类型，返回值类型由函数体中的 `返回` 语句推断；
   - 如果函数体中没有任何 `返回` 语句，则默认返回类型为 `空`（空类型）；
   - 支持的返回类型包括：`整数`、`小数`、`布尔`、`字符串`、`无`（空类型）、以及其他自定义类型。
...
2. 语法要点：
   - 使用 `情况 值:` 定义每个分支，注意是**冒号 `:`** 而不是分号 `;`
   - 使用 `默认:` 定义默认分支（可选）
   - 每个分支**不需要大括号 `{}`** 包裹，直接写语句序列
...
4. 注意事项：
   - 不支持多个值共享同一分支的写法（如 C 语言中的 `case 1: case 2:`）
   - 默认分支是可选的，但建议添加以提高代码健壮性
   - 若故意省略 `中断;`，会导致贯穿执行下一个分支（但不推荐这种做法）
## 8. 模块与文件组织（如使用时）
