...
- **IR 设计**：
  - 采用简单的三地址码风格，便于 C 代码生成与后续扩展。
  - 支持基础算术（加减乘除模）、比较（等于、不等、大小于）、逻辑（与或非）、内存（ALLOCA/LOAD/STORE/MOV）以及控制流（JUMP/BRANCH/CALL/RET）。
  - 每个函数由一组基本块（BasicBlock）组成，通过跳转指令形成控制流图（CFG）。
  - 指令采用 `%rN` 虚拟寄存器表示中间值，映射到 C 语言局部变量。

- **IR Pass**：
  - **常量折叠**：在 IR 层级对已知常量的运算进行提前计算。
  - **死代码删除**：基于 CFG 可达性分析移除不可达的基本块。

- **C 代码生成**：
  - **寄存器映射**：将 IR 虚拟寄存器映射为 C 函数头部的局部变量声明（如 `long long r0, r1;`）。
  - **标识符映射**：中文标识符通过 UTF-8 字节流匹配映射到 C 函数名（如 `主程序` -> `main`, `打印` -> `cn_rt_print_string`）。
  - **结构映射**：为每个 CN_Language 函数生成对应 C 函数，函数体按基本块顺序生成，并使用 `goto` 标签实现控制流跳转。
