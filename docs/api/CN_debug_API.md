...
```c
typedef enum {
    CN_DEBUG_LEVEL_NONE = 0,    // 不输出任何调试信息
    CN_DEBUG_LEVEL_ERROR = 1,   // 错误信息
    CN_DEBUG_LEVEL_WARN = 2,    // 警告信息
    CN_DEBUG_LEVEL_INFO = 3,    // 一般信息
    CN_DEBUG_LEVEL_DEBUG = 4    // 调试信息
} CN_DebugLevel;
```
...
**参数：**
- `level` - 调试级别（CN_DebugLevel枚举值）
- `format` - 格式化字符串
- `...` - 可变参数列表

**示例：**
```c
CN_DEBUG(CN_DEBUG_LEVEL_INFO, "变量值: %d", value);
CN_DEBUG(CN_DEBUG_LEVEL_ERROR, "文件打开失败: %s", filename);
```
...
**示例：**
```c
cn_debug_init(CN_DEBUG_LEVEL_DEBUG);  // 输出所有级别的调试信息
cn_debug_init(CN_DEBUG_LEVEL_ERROR);  // 只输出错误信息
cn_debug_init(CN_DEBUG_LEVEL_NONE);   // 不输出任何调试信息
```

### cn_debug_set_output_file
设置调试信息输出文件。
...
**示例：**
```c
cn_debug_set_output_file("debug.log");  // 输出到文件
cn_debug_set_output_file(NULL);         // 恢复到标准错误
```
...
**示例：**
```c
printf("当前级别: %s\n", cn_debug_level_to_string(CN_DEBUG_LEVEL_INFO));
// 输出: 当前级别: INFO
```
...
int main()
{
    ...
    CN_DEBUG(CN_DEBUG_LEVEL_DEBUG, "初始化变量: count=%d", 42);
    
    // 错误处理
    FILE* file = fopen("nonexistent.txt", "r");
    if (file == NULL)
    {
        CN_DEBUG(CN_DEBUG_LEVEL_ERROR, "无法打开文件: nonexistent.txt");
    }
    ...
    *ptr = 100;
    ...
    CN_DEBUG(CN_DEBUG_LEVEL_INFO, "应用程序正常退出");
