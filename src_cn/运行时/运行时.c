...
char* 复制字符串副本(const char* cn_var_源);
char* 字符串格式(const char* cn_var_格式);
long long 字符串格式化(const char* cn_var_缓冲区, long long cn_var_大小, const char* cn_var_格式);
char* cn_rt_read_line();
...
char* 获取程序名称();
char* 查找选项(const char* cn_var_选项名);
long long 选项存在(const char* cn_var_选项名);
