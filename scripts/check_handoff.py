...
检查面：
  ...
  plans/021   — ①「最后更新：第」戳恰好 1 段（一分区一记录——曾膨胀至 51 段叠加/300KB）
                ...
                ④ §三 队列区禁止残留完成标记（481-a「只列未完成」）；📣（待裁决）态禁已裁决内容
                   （原 §3-B 铁律状态化·580-a）
                ⑤ 唯一任务号体系（580-a·用户批）：§三+§4.2 首列 NNN/NNN-NNN 全局唯一（退役不复用）+
                   孤儿子号拦截+「当前最大任务号」锚行恰 1 处且=实际最大号
...
总表_呈报态_完成标记们 = ("已裁决", "已批", "已选定", "✅")
# ^ §三 📣（待用户裁决）态生命周期铁律（580-a 改立·前身为 §3-B 区铁律 2026-09-16 用户令）：
#   条目经用户裁决必须**当轮改状态**——裁决结论写进行内、状态脱离 📣（转入对应态或销项迁 §4.2）；
#   「✅ 用户裁决…以下为呈报原文留档」式留档同属违规（历史由 git/plans/019/020 承载，
#   📣 态只准有「等用户裁决」状态的条目）。
...
# ^ 队列纪律②（2026-09-21 用户批方案甲·580-a 六态化）：凡带 🔄/⬜/⏸/📣/🔬 的行必须写明「下一棒」
#   （下一步动作与归属——待认领/挂等 X 回签后做 Y/随 Z 轮/重启触发），治「只有状态没有下一棒、
#   接手会话无从接力」；🏃（在飞）豁免——在飞条目的下一棒天然=集成回签。
总表_任务号模式 = re.compile(r"^\|\s*(\d{3}(?:-\d{3})?)\s*\|")
# ^ §三 平表/§4.2 台账首列任务号（580-a 唯一编号体系）：`NNN` 或子任务 `NNN-NNN`；
#   旧编号（T100/D42甲 等带字母前缀形态）不匹配=天然豁免唯一性校验（历史台账封存不改）。
总表_子号模式 = re.compile(r"^(\d{3})-(\d{3})$")
总表_最大号锚模式 = re.compile(r"当前最大任务号[=＝]\s*(\d{3})")
# ^ §三 头注锚行「当前最大任务号=NNN」——三机取号=max+1 的机械锚，校验其与全表实际最大号一致。
看板_机器名们 = ("家机 win-x64", "深度机 linux-x86_64", "单位机 ARM64")
看板_行宽上限 = 300      # 状态板行宽（230-a 指针化后 150~220；旧态最长 1387 字符=收工小作文）


def 查总表呈报态(行们: list[str]) -> list[str]:
    """plans/021 §三 📣（待用户裁决）态生命周期铁律检查（580-a 改立·前身为 §3-B 区检查——
    A~G 分区废除后呈报态由状态列承载，铁律随状态化）：
    队列区内带 📣 的行禁止出现裁决完成标记（已裁决/已批/已选定/✅）——
    条目一经裁决必须当轮改状态（结论留行内、状态脱离 📣 或销项迁 §4.2），禁止「原文留档」。
    """
    问题: list[str] = []
    在队列区 = False
    for 序, 行 in enumerate(行们, 1):
        ...
        命中 = [标记 for 标记 in 总表_呈报态_完成标记们 if 标记 in 检查文本]
        if 命中:
            问题.append(f"plans/021 §三 📣（待用户裁决）态 L{序} 出现裁决完成标记「{'」「'.join(命中)}」"
                        "——已裁决条目必须当轮改状态（裁决结论写进行内、状态脱离 📣 或销项迁 §4.2），"
                        "呈报态禁留任何已裁决内容（含「原文留档」·原 §3-B 铁律状态化）："
                        f"{行.strip()[:60]}")
    return 问题


def 查总表任务号(行们: list[str]) -> list[str]:
    """plans/021 唯一任务号体系检查（580-a 新增·用户批：全局唯一序号 001 起+子号 NNN-NNN 两级）：
    ① §三 平表+§4.2 台账首列任务号（`NNN`/`NNN-NNN` 形态）全局唯一——含「销项编号退役不复用」
      的机械保证（§4.2 已占用号 §三 不得复用；旧编号带字母前缀形态天然不匹配=历史封存豁免）；
    ② 子任务号 `NNN-NNN` 的父号必须存在于 §三（孤儿拦截）；
    ③ §三 头注锚行「当前最大任务号=NNN」必须与全表实际最大父号一致（三机取号=max+1 的锚）；
    ④ 锚行必须存在且恰好 1 处。
    """
    问题: list[str] = []
    在队列区 = False
    ...
    for 序, 行 in enumerate(行们, 1):
        ...
        号 = 匹配.group(1)
        if 号 in 号到行:
            问题.append(f"plans/021 任务号「{号}」重复出现 L{号到行[号]} 与 L{序}"
                        "（全局唯一·退役不复用——销项号不得复用·取号=全表最大+1）："
                        f"{行.strip()[:60]}")
        else:
            号到行[号] = 序
        if 在队列区:
            ...
            if 子:
                队列父号们.add(子.group(1))
    # ② 孤儿子号：父号必须在 §三（父任务自身可以是任意态，但必须存活于队列——
    #    父销项时子任务同轮随迁，故 §4.2 出现子号而 §三 无父号=族完整性破坏）
    for 号, 序 in sorted(号到行.items()):
        子 = 总表_子号模式.match(号)
        if 子 and 子.group(1) not in 号到行:
            问题.append(f"plans/021 子任务号「{号}」L{序} 的父号「{子.group(1)}」不存在于全表"
                        "（孤儿子号——父任务销项时子任务须同轮随迁·限两级）："
                        f"{行们[序 - 1].strip()[:60]}")
    # ③④ 锚行：存在、恰 1 处、数值=实际最大父号
    锚们 = [(序, 总表_最大号锚模式.search(行)) for 序, 行 in enumerate(行们, 1)]
    锚们 = [(序, m) for 序, m in 锚们 if m]
    if len(锚们) != 1:
        问题.append(f"plans/021「当前最大任务号=NNN」锚行出现 {len(锚们)} 处（应恰 1 处"
                    "——§三 头注锚行=三机取号唯一依据）："
                    + "；".join(f"L{序}" for 序, _ in 锚们[:4]))
    else:
        实际最大 = max(int(号.split("-")[0]) for 号 in 号到行) if 号到行 else 0
        锚值 = int(锚们[0][1].group(1))
        if 锚值 != 实际最大:
            问题.append(f"plans/021 锚行 L{锚们[0][0]}「当前最大任务号={锚值:03d}」与全表实际最大号"
                        f"{实际最大:03d} 不一致（取号=max+1 依赖锚准确·新任务立账后须同步刷新锚行）")
    return 问题
...
def 查总表队列区(行们: list[str]) -> list[str]:
    ...
    for 序, 行 in enumerate(行们, 1):
        ...
        if 行.startswith("|") and 总表_队列_状态符号模式.search(检查文本):
            # 「下一棒」判定（580-a 列感知）：六列平表下一棒=第 5 列（split 首元素为空串，
            # 列们[5]）；行内自由写法「下一棒=」仍兼容（表头行含列名字面·天然通过）
            列们 = [列.strip() for 列 in 行.split("|")]
            有下一棒 = "下一棒" in 检查文本 or (len(列们) > 5 and 列们[5])
            if not 有下一棒:
                问题.append(f"plans/021 §3 队列区 L{序} 状态符号行缺「下一棒」（第 5 列空且无行内"
                            "字样）——凡 🔄/⬜/⏸/📣/🔬 行必须写明下一步动作与归属（待认领/挂等X回签后做Y/"
                            "随Z轮/重启触发；方案甲·队列纪律②）："
                            f"{行.strip()[:60]}")
    return 问题
...
def 查总表() -> list[str]:
    ...
    # ④ §三 📣 态生命周期铁律 + §三 队列区完成标记残留（481-a）+ 唯一任务号体系（580-a）
    问题 += 查总表呈报态(行们)
    问题 += 查总表队列区(行们)
    问题 += 查总表任务号(行们)
    return 问题
