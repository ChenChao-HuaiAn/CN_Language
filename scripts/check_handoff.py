...
def 查重复内容行() -> list[str]:
    """长内容行整行重复检测（803 立·lessons 206「新事故同轮扩门禁」·343-a 同族第二例）。

    事故：plans/021 §六 整节替换时 old_string 未包全节——残缺半份表格（**无节标题**）
    滞留 10 行，其中 9 行与完整版逐字重复；「查重复节标题」只认 `## ` 标题，
    无标题残段绕过=假绿。长内容行（≥30 字）在文件内出现 ≥2 次=「追加而非替换」
    事故签名，直接拦截。
    """
    问题: list[str] = []
    for 名 in ("HANDOFF.md", "更新日志.md", "plans/021-任务进度观察表.md",
        ...
        if not 路径.exists():
            continue
        内容行们 = [行.strip() for 行 in 路径.read_text(encoding="utf-8").splitlines()
                  if len(行.strip()) >= 30 and not 行.strip().startswith("|--")]
        for t in sorted({t for t in 内容行们 if 内容行们.count(t) > 1}):
            问题.append(f"{名} 出现重复内容行（×{内容行们.count(t)}）：{t[:50]}…——"
                        "「追加而非替换」事故签名（无标题残段也拦·803 立·343-a 同族）")
    return 问题
