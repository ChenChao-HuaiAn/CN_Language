...
# B3 口径排除（2026-09-16 用户裁决·plans/026 M1 注记）：前 7 格属「低语义密度」——规范条文不定义
# 独立可测的新行为（行为由邻近格用例锚定：287/288/302/274/301 等，非删测试）。
# 判据：优先级=求值语义非拒绝规则（4.5）；EBNF=语法记法非独立功能（4.6.2）；续行/别名=合法性
# 规则无负例形态（4.6#规则2/3.1）；边界=关键字/移除词边界由正例+负例对承载（2.1/2.2）。
# 第 8 格（2.1 正例）为 573 裁决甲连带簿记（603 深度机·机制级）：关键字穷举正例 89 依赖废弃
# v1 词法组件随裁决甲下线（2026-09-22 用户裁决·呈报方案乙「迁移 v2 等价」未采纳）——关键字
# 合法性由 2.1a 正例对+301/302 拒绝负例+全量用例面承载；v2 等价穷举正例若后续补位应移除本条。
EXCLUDED_CELLS = (
    ("2.1", "边界例"),   # 关键字分类表：关键字边界（合法性由 301 拒绝负例承载·穷举正例已随 573 甲下线）
    ...
    ("2.1", "正例"),     # 573 裁决甲连带（603）：穷举正例 89 已下线·v2 等价补位后应移除本条
)
